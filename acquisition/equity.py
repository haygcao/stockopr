...
class GbbqReader(object):
    def get_df(self, fname):
        ...
        df = pd.DataFrame(data=result, columns=['market', 'code', 'date', 'category',
                                                'liutong_hongli_panqian',
                                                'zongguben_peigujia_qian',
                                                'zongguben_songgu_qian',
                                                'zongguben_peigu_hou'])
        return df
...
def save_equity():
    ...
    with mysqlcli.get_cursor() as c:
        try:
            ...
            # c.execute('select max(date) date from equity')
            # lastest_date = c.fetchone()['date']
            # if lastest_date:
            #     df = df[df['date' > lastest_date]]
            #     if df.empty:
            #         return

            c.execute('truncate table equity')

            engine = create_engine('mysql+pymysql://{0}:{1}@127.0.0.1:3306/stock?charset=utf8mb4'.format(config.db_user, config.db_passwd))

            df.to_sql(name='equity', con=engine, if_exists='append', index=False, chunksize=20000)
