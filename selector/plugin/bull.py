...
def bull_ma_one_day(quote, mas, first_n_ma, back_day):
    keys = list(mas.keys())
    keys.sort()
    keys = keys[:first_n_ma]
    l = [mas[k][-back_day - 1] for k in keys]
...
# r: 取maN的后n个
def bull_ma(quote, begin=config.DT_DAY, last_n_maN=config.DT_LAST_N_MA):
...
# ma5 - ma60 > 5%
def bull_ma2(quote, p=config.DT_MIN_UP_P):
...
def bull_boll(quote, b=config.DT_BOLL_DAY, d=config.DT_BOLL_DAY_AGO):
    r = bbands(quote)
...
# 5天前的sar
def bull_sar(quote, b=config.DT_SAR_DAY):
    r = sar(quote)
    b *= -1
...
def bull(quote):
    if not bull_ma(quote):
        return False
    if not bull_dmi(quote):
        return False
    if not bull_macd(quote):
        return False
    if not bull_sar(quote):
        return False
